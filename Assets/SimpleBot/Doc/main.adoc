...
`condition` is useful for controlling the discourse between SimpleBot and users, but it
is still inadequate to express complex condition. SimpleBot supports `boolean condition` to
express complex conditions.

==== Boolean conditions

`SimpleBot` supports the combinations of conditions with boolean conditions (`should` and `must`).
