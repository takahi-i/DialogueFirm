...
`ReplySentence` return the sentence for input sentences. The following function, `Reply` loads the input
...
`BotEngine` has a method to return `State` object which contains the status of `BotEngine`.
The following sample uses the `State` method to switches the images of an application.
