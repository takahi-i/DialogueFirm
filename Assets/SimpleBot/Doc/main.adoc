...
TIP: The matched slot key and value are used in the condition block of the responder.
...
The slot value in template matching is automatically set to state.
...
The following setting contains two intents `failed` and `succeeded`. They match keywords `fail` and `fixed` respectively.
